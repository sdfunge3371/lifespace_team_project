...
public class FaqRedisService {
	...
	// StringRedisTemplate : Spring 內建的 Redis 操作工具類，專門用來操作字串格式的資料
		...
		// --------將FAQ資料寫入Redis的Hash結構（用 JSON 保存）--------
		public void addFaqWithId(FaqHashtag hashtag, String faqId, String question, String answer) {
			String redisKey = "faq:" + hashtag.name();

			// 建立一個 Map，將問題與答案包裝成JSON
			Map<String, String> faqData = new HashMap<>();
			faqData.put("question", question);
			faqData.put("answer", answer);

			try {
				// 轉成 JSON 字串（格式：{"question":"...","answer":"..."})
				String valueAsJson = new ObjectMapper().writeValueAsString(faqData);

				// 存進 Redis（field = FAQ01, value = JSON 字串）
				redisTemplate.opsForHash().put(redisKey, faqId, valueAsJson);

			} catch (Exception e) {
				System.err.println("Redis FAQ JSON 轉換失敗：" + e.getMessage());
			}
		}

		// --------呼叫addFaqWithId()寫入多筆資料進Redis(這是資料初始化時，上線會關閉)--------
		public void initFaqData() {

			// 先清空所有Redis中的FAQ資料
			FaqHashtag[] tags = FaqHashtag.values();
			for (int i = 0; i < tags.length; i++) {
				FaqHashtag tag = tags[i];
			...
			addFaqWithId(FaqHashtag.RENTAL, "FAQ03", "租賃空間如何計價",
			...
			addFaqWithId(FaqHashtag.PAYMENT, "FAQ10", "付款以及發票的開立方式？",
			...
			addFaqWithId(FaqHashtag.OTHER, "FAQ11", "防詐騙資訊提醒",
			...
			addFaqWithId(FaqHashtag.RENTAL, "FAQ06", "LifeSpace 提供哪些公用設備與設施？",
			...
			addFaqWithId(FaqHashtag.USAGE, "FAQ07", "使用規範",
			...
			addFaqWithId(FaqHashtag.OTHER, "FAQ08", "注意事項",
			...
			addFaqWithId(FaqHashtag.OTHER, "FAQ04", "自助式空間如何清潔？",
					"使用結束後，請遵守「使用後恢復原狀」原則，讓下一位使用者也能擁有舒適的環境！\n" + " 清潔步驟如下：\n" + " 垃圾分類丟棄（垃圾桶位置請參考現場指引）。\n"
							+ " 擦拭桌面 & 白板，恢復使用前狀態。\n" + " 歸位家具 & 設備（如桌椅、遙控器等）。\n" + " 關閉電燈 & 冷氣，節約能源。\n"
							+ " *若未完成清潔，可能會收取 NT$600-NT$5,000清潔費，請共同維護空間品質！");

			System.out.println("Redis FAQ初始化完成，共匯入 " + faqCount + " 筆FAQ");
		}

		// ------取得redis對應的Hash結構（會排除掉 MySQL 中下架的項目）------
		public Map<String, String> getFaqByHashtag(String displayName) {
			...
			// 從 Redis Hash 中取得所有 FAQ（欄位=FAQ_ID|問題、值=回答）
			Map<Object, Object> redisData = redisTemplate.opsForHash().entries(redisKey);

			// 轉成 Map<String, String> （Redis 會回傳 Object型別）
			Map<String, String> result = new HashMap<>();
			for (Map.Entry<Object, Object> entry : redisData.entrySet()) {
				String faqId = entry.getKey().toString();
				String jsonString = entry.getValue().toString();

				try {
					// ObjectMapper: json 轉換工具，完成 json和Java 的Object 的互相轉換
					ObjectMapper mapper = new ObjectMapper();
					// TypeReference：幫 Jackson 確認泛型參數，告訴Jackson目標型別是 Map<String,String>
					Map<String, String> faqJson = mapper.readValue(jsonString, new TypeReference<Map<String, String>>() {
					});

					// 檢查FAQ是否存在且為上架中
					Faq faq = repository.findById(faqId).orElse(null);
					if (faq != null && faq.getFaqStatus() == 1) {
						result.put(faqId, jsonString); // 加入結果
					}
				} catch (Exception e) {
					System.err.println("FAQ JSON 解析錯誤：" + e.getMessage());
				}
			}
			return result;
		}
